...
export default function AdminHome() {
  ...
  const platformRevenue = data.bookings.filter((booking) => booking.status === "completed").reduce((sum, booking) => sum + booking.platformFeeAmount, 0);
  ...
  return (
    <Screen>
      ...
      <View style={styles.grid}>
        <Metric label="Organisations" value={String(data.organisations.length)} /><Metric label="Trainers" value={String(data.trainers.length)} /><Metric label="Verified trainers" value={String(verifiedTrainers)} /><Metric label="Open shifts" value={String(openShifts)} /><Metric label="Booked shifts" value={String(bookedShifts)} /><Metric label="Completed shifts" value={String(completedShifts)} />
      </View>
      <InfoCard title="Platform revenue" subtitle={`${formatCurrency(platformRevenue)} from completed seeded bookings.`} />
      <InfoCard title="Pending manual verification">{data.credentials.filter((credential) => credential.status === "pending").map((credential) => <Text key={credential.id} style={styles.note}>{credential.displayName} requires admin review.</Text>)}</InfoCard>
...
const styles = StyleSheet.create({
  ...
  metric: { width: "47%", borderColor: colors.border, borderRadius: 8, borderWidth: 1, backgroundColor: colors.surface, padding: 14 },
