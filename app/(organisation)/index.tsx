...
export default function OrganisationHome() {
  ...
  return (
    <Screen>
      ...
      <View style={styles.metrics}><Metric label="Active shifts" value={String(activeShifts.length)} /><Metric label="Est. spend" value={formatCurrency(totalSpend)} /></View>
      ...
      {shifts.map((shift) => (
        <InfoCard key={shift.id} title={shift.title} subtitle={`${shift.date}, ${shift.startTime} - ${shift.endTime} | ${shift.location}`}>
          <View style={styles.row}><StatusBadge label={shift.status.replace("_", " ")} /><Text style={styles.money}>{formatCurrency(shift.trainerPay)} trainer pay</Text></View>
