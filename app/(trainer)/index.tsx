...
export default function TrainerHome() {
  ...
  return (
    <Screen>
      ...
      <View style={styles.metrics}><Metric label="Rating" value={trainer.averageRating.toFixed(1)} /><Metric label="Reliability" value={`${trainer.reliabilityScore}`} /></View>
      <InfoCard title="Verification status">
        ...
        {credentials.map((credential) => <Text key={credential.id} style={styles.note}>{credential.displayName}: {credential.status}</Text>)}
      </InfoCard>
      {openShifts.map((shift) => (
        <InfoCard key={shift.id} title={shift.title} subtitle={`${shift.sport} | ${shift.date}, ${shift.startTime} - ${shift.endTime}`}>
          <Text style={styles.note}>{shift.location}</Text><Text style={styles.money}>{formatCurrency(shift.trainerPay)} trainer pay</Text><Text style={styles.note}>Application actions are scheduled for Stage 2.</Text>
        </InfoCard>
      ))}
...
const styles = StyleSheet.create({
  ...
  metric: { flex: 1, borderColor: colors.border, borderRadius: 8, borderWidth: 1, backgroundColor: colors.surface, padding: 14 },
